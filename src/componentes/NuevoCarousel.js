...
class NuevoCarousel extends Component {
  constructor(props) {
    super(props);
    this.state = {
      currentIndex: 0,
  ...
  fetchProductos = async () => {
    try {
      ...
      // Filtrar los productos que son nuevos (nuevo === true)
      const productosNuevos = data.filter(
        (producto) => producto.nuevo === true
      );

      this.setState({ productos: productosNuevos });
    } catch (error) {
